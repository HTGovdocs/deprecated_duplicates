...
get_vals_sql = "SELECT gp.prop, gp.val, COUNT(gp.val) AS cv FROM mwarin_ht.gd_prop AS gp JOIN mwarin_ht.gd_str AS gs ON (gp.prop = gs.gd_str_id) WHERE gs.str = ? GROUP BY gp.prop, gp.val HAVING cv >= 2"
...
["DELETE FROM mwarin_ht.gd_item_cluster", 
 "DELETE FROM mwarin_ht.gd_cluster"].each do |dq|
  puts dq;
  @conn.update(dq);
end
